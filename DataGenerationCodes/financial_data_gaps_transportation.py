...
# Configuration
SECTOR = 'Transportation and Public Utilities'
USE_WEB_SEARCH = False  # Toggle for live benchmarks (slower) vs pre-populated (faster)
BENCHMARK_CACHE_FILE = 'transportation_benchmark_cache.json'
LOG_FILE = 'transportation_financials.log'
...
# Pre-populated benchmark means (in USD) for Transportation and Public Utilities sector
# Transportation and Public Utilities characteristics:
# - Infrastructure-heavy with high fixed costs
# - Regulated industries with stable but moderate margins
# - High asset values due to infrastructure and equipment
# - Significant debt levels due to capital requirements
# - Stable cash flows due to essential services
# - Technology investments for efficiency and modernization
pre_populated_benchmarks = {
    f"average revenue for transportation and public utilities sector companies 2024": 2.5e9,  # Stable revenue from essential services
    f"average expenses for transportation and public utilities sector companies 2024": 2.2e9, # High operational and maintenance costs
    f"average net_income for transportation and public utilities sector companies 2024": 3e8, # Moderate margins due to regulation
    f"average assets for transportation and public utilities sector companies 2024": 8e9,     # Very asset-heavy (infrastructure, equipment)
    f"average liabilities for transportation and public utilities sector companies 2024": 4.5e9, # High debt due to infrastructure investment
    f"average equity for transportation and public utilities sector companies 2024": 3.5e9,   # Higher equity due to regulatory requirements
    f"average operating_cash for transportation and public utilities sector companies 2024": 1.2e9, # Strong, stable cash flow
    f"average investing_cash for transportation and public utilities sector companies 2024": -8e8, # Heavy infrastructure investment
    f"average financing_cash for transportation and public utilities sector companies 2024": 4e8,  # Regular financing for capital projects
    f"average r&d_expense for transportation and public utilities sector companies 2024": 1.2e8,   # Technology for efficiency and modernization
    f"average operating_income for transportation and public utilities sector companies 2024": 5e8  # Operating income before interest/taxes
}

# Distribution types for Transportation and Public Utilities sector components
# Transportation and Public Utilities have unique characteristics:
# - Infrastructure-heavy operations create high asset values
# - Regulatory oversight provides stability but limits margins
# - Essential services create predictable demand patterns
# - Technology investments for modernization and efficiency
# - Long asset lifecycles with significant maintenance costs
sector_distributions = {
    'Transportation and Public Utilities': {
        'revenue': 'lognorm',          # Lognormal due to scale effects and infrastructure size
        'expenses': 'gamma',           # Right-skewed for operational cost variability
        'net_income': 'truncnorm',     # Truncated due to regulatory constraints on margins
        'assets': 'lognorm',           # Lognormal for infrastructure asset distribution
        'liabilities': 'gamma',        # Gamma for debt structure variability
        'equity': 'norm',              # More stable equity due to regulatory requirements
        'operating_cash': 'truncnorm', # Truncated for stable but constrained cash flows
        'investing_cash': 'expon',     # Exponential for infrastructure investments
        'financing_cash': 'norm',      # Normal for regular financing needs
        'r&d_expense': 'weibull',      # Skewed for technology and modernization investments
...
def get_distribution_params(sector, component):
    ...
    sd = mean * 0.22  # Transportation has lower variability (22%) due to regulatory stability
    return mean, sd
...
def update_table(sector, table_name, columns):
    ...
    # Fetch Transportation and Public Utilities companies (SIC 4000-4999)
    df_sub = pd.read_sql("SELECT adsh, sic FROM submissions WHERE sic BETWEEN 4000 AND 4999", conn)
    if df_sub.empty:
...
def main():
    """Generate financial data for Transportation and Public Utilities sector."""
    ...
    # Update submissions with sector for Transportation and Public Utilities (SIC 4000-4999)
    cursor.execute("""
        UPDATE submissions
        SET sector = 'Transportation and Public Utilities'
        WHERE sic BETWEEN 4000 AND 4999
    """)
    conn.commit()
