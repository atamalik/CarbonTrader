...
BENCHMARK_CACHE_FILE = 'services_benchmark_cache.json'
LOG_FILE = 'services_financials.log'
...
# Pre-populated benchmark means (in USD) for Services sector
# Services sector characteristics:
# - Very diverse sector including professional services, technology, healthcare, etc.
# - Generally lower capital intensity than other sectors
# - Higher margins due to service-based business models
# - Significant investment in human capital and technology
# - Variable revenue patterns depending on service type
# - Lower asset requirements but higher operational costs
pre_populated_benchmarks = {
    f"average revenue for services sector companies 2024": 1.5e9,    # Moderate revenue with high variability
    f"average expenses for services sector companies 2024": 1.2e9,   # High operational costs (labor, technology)
    f"average net_income for services sector companies 2024": 3e8,   # Good margins due to service model
    f"average assets for services sector companies 2024": 2.5e9,     # Lower asset intensity than other sectors
    f"average liabilities for services sector companies 2024": 1.5e9, # Lower debt levels due to lower capital needs
    f"average equity for services sector companies 2024": 1e9,       # Moderate equity levels
    f"average operating_cash for services sector companies 2024": 4e8,  # Good cash flow from services
    f"average investing_cash for services sector companies 2024": -3e8, # Investment in technology and human capital
    f"average financing_cash for services sector companies 2024": 1e8,  # Lower financing needs
    f"average r&d_expense for services sector companies 2024": 2e8,     # High R&D for technology services
    f"average operating_income for services sector companies 2024": 4e8  # Operating income before interest/taxes
}

# Distribution types for Services sector components
# Services sector has unique characteristics:
# - Lower capital intensity creates different asset structures
# - High human capital investment affects cost structures
# - Technology investment is crucial for many service companies
# - Variable revenue patterns depending on service type
# - Generally higher margins due to service-based models
sector_distributions = {
    'Services': {
        'revenue': 'gamma',            # Right-skewed for varying service business sizes
        'expenses': 'gamma',           # Right-skewed for operational cost variability
        'net_income': 'truncnorm',     # Truncated to reflect service margins
        'assets': 'gamma',             # Right-skewed for asset-light service businesses
        'liabilities': 'gamma',        # Lower debt levels create gamma distribution
        'equity': 'norm',              # More stable equity structure
        'operating_cash': 'truncnorm', # Truncated for service cash flow patterns
        'investing_cash': 'expon',     # Exponential for technology and human capital investments
        'financing_cash': 'norm',      # Normal for lower financing needs
        'r&d_expense': 'weibull',      # Skewed for technology and innovation investments
...
def get_distribution_params(sector, component):
    ...
    sd = mean * 0.32  # Services has high variability (32%) due to sector diversity
    return mean, sd
...
def update_table(sector, table_name, columns):
    ...
    # Fetch Services companies (SIC 7000-8999)
    df_sub = pd.read_sql("SELECT adsh, sic FROM submissions WHERE sic BETWEEN 7000 AND 8999", conn)
    if df_sub.empty:
...
def main():
    """Generate financial data for Services sector."""
    ...
    # Update submissions with sector for Services (SIC 7000-8999)
    cursor.execute("""
        UPDATE submissions
        SET sector = 'Services'
        WHERE sic BETWEEN 7000 AND 8999
    """)
    conn.commit()
