...
BENCHMARK_CACHE_FILE = 'finance_benchmark_cache.json'
LOG_FILE = 'finance_financials.log'
...
# Pre-populated benchmark means (in USD) for Finance, Insurance & Real Estate sector
# Finance, Insurance & Real Estate characteristics:
# - Extremely high leverage with massive balance sheets
# - Revenue from interest, fees, and premiums rather than traditional sales
# - High regulatory capital requirements
# - Significant investment in technology and compliance
# - Interest rate sensitivity affects profitability
# - High operational leverage due to fixed cost structures
pre_populated_benchmarks = {
    f"average revenue for finance insurance real estate sector companies 2024": 5e9,      # High revenue from financial services
    f"average expenses for finance insurance real estate sector companies 2024": 4e9,     # High operational and regulatory costs
    f"average net_income for finance insurance real estate sector companies 2024": 1e9,   # Good margins from financial services
    f"average assets for finance insurance real estate sector companies 2024": 50e9,      # Massive balance sheets due to leverage
    f"average liabilities for finance insurance real estate sector companies 2024": 45e9,  # Extremely high leverage
    f"average equity for finance insurance real estate sector companies 2024": 5e9,       # High equity due to regulatory requirements
    f"average operating_cash for finance insurance real estate sector companies 2024": 2e9, # Strong cash flow from operations
    f"average investing_cash for finance insurance real estate sector companies 2024": -1.5e9, # Investment in technology and compliance
    f"average financing_cash for finance insurance real estate sector companies 2024": 1e9,  # Regular financing for balance sheet growth
    f"average r&d_expense for finance insurance real estate sector companies 2024": 3e8,     # High tech investment for fintech
    f"average operating_income for finance insurance real estate sector companies 2024": 1.5e9  # Operating income before interest/taxes
}

# Distribution types for Finance, Insurance & Real Estate sector components
# Finance, Insurance & Real Estate have unique characteristics:
# - Extremely high leverage creates massive balance sheets
# - Interest rate sensitivity affects revenue and profitability
# - Regulatory oversight provides stability but constrains growth
# - Technology investment is crucial for competitiveness
# - High operational leverage due to fixed cost structures
sector_distributions = {
    'Finance, Insurance & Real Estate': {
        'revenue': 'lognorm',          # Lognormal due to scale effects and interest rate sensitivity
        'expenses': 'norm',            # Normal for operational cost structures
        'net_income': 'norm',          # Normal for profitability patterns
        'assets': 'monte_carlo',       # Monte Carlo for complex balance sheet structures
        'liabilities': 'monte_carlo',  # Monte Carlo for complex liability structures
        'equity': 'norm',              # Normal for regulatory capital requirements
        'operating_cash': 'truncnorm', # Truncated for cash flow constraints
        'investing_cash': 'expon',     # Exponential for technology and compliance investments
        'financing_cash': 'norm',      # Normal for regular financing needs
        'r&d_expense': 'weibull',      # Skewed for fintech and innovation investments
        'operating_income': 'lognorm'  # Lognormal for operating income (scale effects)
    }
}
...
def get_distribution_params(sector, component):
    ...
    sd = mean * 0.20  # Finance has lower variability (20%) due to regulatory oversight
    return mean, sd
...
def generate_value(sector, component, mean, sd):
    ...
    try:
        if dist_type == 'lognorm':
            ...
            # Monte Carlo simulation for complex balance sheet structures
            iterations = 100  # Reduced for speed
            simulations = np.array([
                norm(loc=mean * np.random.uniform(0.8, 1.2), scale=sd * np.random.uniform(0.9, 1.1)).rvs() 
...
def update_table(sector, table_name, columns):
    ...
    # Fetch Finance, Insurance & Real Estate companies (SIC 6000-6799)
    df_sub = pd.read_sql("SELECT adsh, sic FROM submissions WHERE sic BETWEEN 6000 AND 6799", conn)
    if df_sub.empty:
...
def main():
    """Generate financial data for Finance, Insurance & Real Estate sector."""
    ...
    # Update submissions with sector for Finance, Insurance & Real Estate (SIC 6000-6799)
    cursor.execute("""
        UPDATE submissions
        SET sector = 'Finance, Insurance & Real Estate'
