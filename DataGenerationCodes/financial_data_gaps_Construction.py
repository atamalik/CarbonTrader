...
import pandas as pd
import numpy as np
from scipy.stats import lognorm, gamma, norm, weibull_min, truncnorm
import requests
...
BENCHMARK_CACHE_FILE = 'benchmark_cache.json'
LOG_FILE = 'construction_financials.log'
...
# Pre-populated benchmark means (in USD) for Construction sector
pre_populated_benchmarks = {
    f"average revenue for construction sector companies 2024": 1e9,
    f"average expenses for construction sector companies 2024": 9e8,
    f"average net_income for construction sector companies 2024": 1e8,
    f"average assets for construction sector companies 2024": 2e9,
    f"average liabilities for construction sector companies 2024": 1.2e9,
    f"average equity for construction sector companies 2024": 8e8,
    f"average operating_cash for construction sector companies 2024": 3e8,
    f"average investing_cash for construction sector companies 2024": -2e8,
    f"average financing_cash for construction sector companies 2024": 1e8,
    f"average r&d_expense for construction sector companies 2024": 5e7,
    f"average operating_income for construction sector companies 2024": 1.5e8
}

# Distribution types for Construction sector components
sector_distributions = {
    'Construction': {
        'revenue': 'lognorm',        # Skewed for varying project sizes
        'expenses': 'gamma',        # Right-skewed for high variability
        'net_income': 'truncnorm',  # Truncated to avoid extreme negatives
        'assets': 'gamma',          # Right-skewed for asset-heavy firms
        'liabilities': 'gamma',     # Skewed due to debt variability
        'equity': 'norm',           # Normally distributed
        'operating_cash': 'truncnorm',  # Truncated to reflect cash flow constraints
        'investing_cash': 'norm',   # Normal for investments in equipment
        'financing_cash': 'norm',   # Normal for financing activities
        'r&d_expense': 'weibull',   # Skewed for occasional R&D in construction tech
        'operating_income': 'lognorm'  # Skewed for profitability variations
    }
}
...
def get_distribution_params(sector, component):
    ...
    sd = mean * 0.3  # Assume SD is 30% of mean for variability
    return mean, sd
...
def update_table(sector, table_name, columns):
    ...
    # Fetch Construction companies (SIC 1500-1799)
    df_sub = pd.read_sql("SELECT adsh, sic FROM submissions WHERE sic BETWEEN 1500 AND 1799", conn)
    if df_sub.empty:
...
def main():
    """Generate financial data for Construction sector."""
    ...
    # Update submissions with sector for Construction (SIC 1500-1799)
    cursor.execute("""
        UPDATE submissions
        SET sector = 'Construction'
        WHERE sic BETWEEN 1500 AND 1799
    """)
    conn.commit()
