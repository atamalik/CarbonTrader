...
# Pre-populated benchmark means (in USD) for Agriculture sector
pre_populated_benchmarks = {
    f"average revenue for agriculture sector companies 2024": 5e8,
    f"average expenses for agriculture sector companies 2024": 4e8,
    f"average net_income for agriculture sector companies 2024": 1e8,
    f"average assets for agriculture sector companies 2024": 1e9,
    f"average liabilities for agriculture sector companies 2024": 5e8,
    f"average equity for agriculture sector companies 2024": 5e8,
    f"average operating_cash for agriculture sector companies 2024": 1.5e8,
    f"average investing_cash for agriculture sector companies 2024": -1e8,
    f"average financing_cash for agriculture sector companies 2024": 5e7,
    f"average r&d_expense for agriculture sector companies 2024": 2e7,
    f"average operating_income for agriculture sector companies 2024": 1.2e8
}

# Distribution types for Agriculture sector components
sector_distributions = {
    'Agriculture': {
        'revenue': 'lognorm',
        'expenses': 'gamma',
        'net_income': 'truncnorm',
        'assets': 'gamma',
        'liabilities': 'gamma',
        'equity': 'norm',
        'operating_cash': 'truncnorm',
        'investing_cash': 'norm',
        'financing_cash': 'norm',
        'r&d_expense': 'weibull',
        'operating_income': 'lognorm'
    }
}
...
def update_table(sector, table_name, columns):
    ...
    # Fetch Agriculture companies (SIC 100-999)
    df_sub = pd.read_sql("SELECT adsh, sic FROM submissions WHERE sic BETWEEN 100 AND 999", conn)
    if df_sub.empty:
...
def main():
    """Generate financial data for Agriculture sector."""
    ...
    # Update submissions with sector for Agriculture (SIC 100-999)
    cursor.execute("""
        UPDATE submissions
        SET sector = 'Agriculture'
        WHERE sic BETWEEN 100 AND 999
    """)
    conn.commit()
