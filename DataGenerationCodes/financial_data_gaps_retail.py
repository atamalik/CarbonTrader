...
BENCHMARK_CACHE_FILE = 'retail_benchmark_cache.json'
LOG_FILE = 'retail_financials.log'
...
# Pre-populated benchmark means (in USD) for Retail Trade sector
# Retail trade characteristics:
# - High revenue but lower margins than wholesale
# - Inventory management is crucial
# - Seasonal variations in cash flows
# - High competition leading to thin margins
# - Significant investment in store operations and technology
pre_populated_benchmarks = {
    f"average revenue for retail trade sector companies 2024": 4e9,      # High volume, competitive pricing
    f"average expenses for retail trade sector companies 2024": 3.5e9,   # High cost of goods sold + operating expenses
    f"average net_income for retail trade sector companies 2024": 5e8,   # Thin margins due to competition
    f"average assets for retail trade sector companies 2024": 6e9,       # High inventory + store assets
    f"average liabilities for retail trade sector companies 2024": 3.5e9, # High working capital needs
    f"average equity for retail trade sector companies 2024": 2.5e9,     # Moderate equity levels
    f"average operating_cash for retail trade sector companies 2024": 1e9,  # Good cash flow from sales
    f"average investing_cash for retail trade sector companies 2024": -7e8, # Heavy investment in stores/tech
    f"average financing_cash for retail trade sector companies 2024": 3e8,  # Moderate financing needs
    f"average r&d_expense for retail trade sector companies 2024": 1.5e8,   # Tech investment for e-commerce/operations
    f"average operating_income for retail trade sector companies 2024": 6e8  # Operating income before interest/taxes
}

# Distribution types for Retail Trade sector components
# Retail trade has distinct characteristics:
# - High revenue volatility due to seasonality
# - Inventory management creates asset variability
# - Competitive pressures affect margins
# - Technology investments for omnichannel operations
# - Store expansion/contraction cycles
sector_distributions = {
    'Retail Trade': {
        'revenue': 'lognorm',          # Lognormal for revenue distribution (some very large retailers)
        'expenses': 'gamma',           # Right-skewed for high cost variability
        'net_income': 'truncnorm',     # Truncated to reflect competitive thin margins
        'assets': 'lognorm',           # Lognormal for asset distribution (inventory + stores)
        'liabilities': 'gamma',        # Working capital and lease obligations create variability
        'equity': 'norm',              # More stable equity structure
        'operating_cash': 'truncnorm', # Good cash flow but with seasonal constraints
        'investing_cash': 'expon',     # Exponential for store/technology investments
        'financing_cash': 'norm',      # Normal for moderate financing needs
        'r&d_expense': 'weibull',      # Skewed for technology/e-commerce investments
...
def get_distribution_params(sector, component):
    ...
    sd = mean * 0.28  # Retail has moderate variability (28%) - more than wholesale (25%) but less than construction (30%)
    return mean, sd
...
def update_table(sector, table_name, columns):
    ...
    # Fetch Retail Trade companies (SIC 5200-5999)
    df_sub = pd.read_sql("SELECT adsh, sic FROM submissions WHERE sic BETWEEN 5200 AND 5999", conn)
    if df_sub.empty:
...
def main():
    """Generate financial data for Retail Trade sector."""
    ...
    # Update submissions with sector for Retail Trade (SIC 5200-5999)
    cursor.execute("""
        UPDATE submissions
